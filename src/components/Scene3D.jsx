...
// Container wireframe - Rectangular or Cylindrical
function Container({ size, isCyl, dia, color = '#b91c1c' }) {
  const [l, h, w] = size

  if (isCyl) {
    const r = (dia / 2) * (l / dia) // Scaled radius
    return (
      <group>
        {/* Base floor - circular */}
        ...
        {/* Container wireframe - cylindrical */}
        ...
        <mesh position={[0, h / 2, 0]} rotation={[0, 0, 0]}>
          <cylinderGeometry args={[r, r, h, 32, 1, true, 0, Math.PI]} />
...
function PackedBoxes({ results, scaleFactor, filterIndex }) {
  const boxes = useMemo(() => {
    ...
    const boxList = []
    const gap = 0.05 // Reduced for tighter look

    results.items.forEach((item, itemIdx) => {
      if (filterIndex !== 'all' && Number(filterIndex) !== itemIdx) return

      // Mixed mode uses exact coordinates calculated in the store
      if (item.packedCoords && item.packedCoords.length > 0) {
        // Defensive: handle both {l,w,h} object and [l,w,h] array formats
        const getDim = (obj, key, index) => {
          if (!obj) return 0;
          if (typeof obj[key] === 'number') return obj[key];
          if (Array.isArray(obj) && typeof obj[index] === 'number') return obj[index];
          return 0;
        };

        const orientL = getDim(item.orient, 'l', 0);
        const orientW = getDim(item.orient, 'w', 1);
        const orientH = getDim(item.orient, 'h', 2);

        const l = orientL * scaleFactor;
        const w = orientW * scaleFactor;
        const h = orientH * scaleFactor;

        item.packedCoords.forEach((coord, i) => {
          // If coord has its own dimensions (mixed mode fix), use them!
          const boxL = (coord.l || orientL) * scaleFactor;
          const boxW = (coord.w || orientW) * scaleFactor;
          const boxH = (coord.h || orientH) * scaleFactor;

          // Add 0.3 floor offset here (not scaled by factor to match floor mesh)
          const yPos = (coord.y * scaleFactor) + 0.3;

          boxList.push({
            key: `mixed-${itemIdx}-${i}`,
            position: [coord.x * scaleFactor, yPos, coord.z * scaleFactor],
            size: [boxL - gap, boxH - gap, boxW - gap],
            color: item.color
          })
        });
        return; // Done processing this item for mixed mode
      }

      // Individual mode logic (legacy)
      // orient is already in CM from the store calculation
      const { orient, nx, ny, nz, color, startH = 0, pos } = item
      ...
      for (let layer = 0; layer < maxLayers; layer++) {
        if (pos) {
          // Cylindrical packing positions
          pos.forEach((p, pi) => {
            const x = p.x * scaleFactor
            const y = startY + layer * boxH + boxH / 2 + 0.3
            const z = p.y * scaleFactor
            boxList.push({
              key: `${itemIdx}-${layer}-${pi}`,
              position: [x, y, z],
              size: [boxL - gap, boxH - gap, boxW - gap],
              color
            })
...
function Scene({ filterIndex }) {
  ...
  return (
    <>
      ...
      <Container
        size={[scaledL, scaledH, scaledW]}
        isCyl={results ? results.renderContainer?.isCyl : storageType === 'drum'}
        dia={results ? results.renderContainer?.dia * scaleFactor : convertToCM(storage.l, storageUnit) * scaleFactor}
      />
    ...
    <div style={{ position: 'relative', width: '100%', height: '100%' }}>
      ...
      >
        ...
        <OrbitControls
          ...
          minDistance={40}
          maxDistance={500}
          target={results && results.renderContainer?.isCyl ? [0, 30, 0] : [50, 30, 50]}
